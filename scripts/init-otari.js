...
async function makeGhost() {
  ...
  if (ghostExists == false) {
    ...
    canvas.primary.addChild(ghostContainer)
    ghostExists = true
...
async function removeGhost() {
  if (ghostExists) {
    ...
    canvas.primary.removeChild(ghostContainer)
...
let updateLoop = function particleUpdater() {
  ...
  leftPrintEmitter.rotate(ghostNull.rotation)
  ...
  rightPrintEmitter.rotate(ghostNull.rotation)
  now = newNow
}
